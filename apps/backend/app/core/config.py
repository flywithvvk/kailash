...
def extract_db_name_from_mongo_url(mongo_url: str, default_name: str = "kailash_aegis") -> str:
    """
    ...
    permissions on a specific database (like 'kailash_aegis'), NOT on 'test'.
    Always default to 'kailash_aegis' unless explicitly specified in URL.
    """
    if not mongo_url:
        return default_name
    ...
    try:
        ...
        # No database specified in URL - use the default (kailash_aegis)
...
# 3. Default to 'kailash_aegis' (NOT 'test' - users don't have permissions)
...
if _explicit_db_name:
    ...
    _database_name = extract_db_name_from_mongo_url(_mongo_url, "kailash_aegis")

class Settings(BaseSettings):
    # App Settings
    APP_NAME: str = "KAILASH AEGIS HUB Backend"
    VERSION: str = "2.0.0"
    DEBUG: bool = False
    ...
    emergent_project_id: Optional[str] = os.environ.get('EMERGENT_PROJECT_ID', 'kailash-aegis-hub')
