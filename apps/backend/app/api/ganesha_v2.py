...
async def orchestrate_request_v2(
    ...
    # Build user context
    user_context = {
        'user_name': getattr(current_user, 'full_name', current_user.aegis_code),
...
async def get_conversation_history_v2(
    ...
    query_filter = {'user_id': current_user.aegis_code}
    if product:
...
async def get_orchestrator_stats_v2(
    ...
    pipeline = [
        {'$match': {'user_id': current_user.aegis_code, 'agent_id': {'$exists': True}}},
