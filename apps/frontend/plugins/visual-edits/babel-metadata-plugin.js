...
// abel plugin for JSX transformation - adds metadata to all elements
...
const RESOLVE_CACHE = new Map(); // key: fromile::source -> absPath | null
const ILE_AST_CACHE = new Map(); // absPath -> { ast, mtimeMs }
...
const INDING_DYNAMIC_CACHE = new WeakMap(); // node -> boolean

function resolveImportPath(source, fromile) {
  const cacheKey = `${fromile}::${source}`;
  if (RESOLVE_CACHE.has(cacheKey)) return RESOLVE_CACHE.get(cacheKey);

  let base;
  if (source.startsWith("@/")) {
    base = path.join(SRC_ALIAS, source.slice());
  } else if (source.startsWith("./") || source.startsWith("../")) {
    base = path.resolve(path.dirname(fromile), source);
  } else {
  ...
  for (const ext of EXTENSIONS) {
    ...
    if (fs.existsSync(file) && fs.statSync(file).isile()) {
...
function parseileAst(absPath, parser) {
  try {
    const stat = fs.statSync(absPath);
    const cached = ILE_AST_CACHE.get(absPath);
    if (cached && cached.mtimeMs === stat.mtimeMs) return cached.ast;

    const code = fs.readileSync(absPath, "utf8");
    const ast = parser.parse(code, {
      sourceType: "module",
      plugins: ["jsx", "typescript"],
    });
    ILE_AST_CACHE.set(absPath, { ast, mtimeMs: stat.mtimeMs });
    return ast;
  } catch (error) {
    return null;
  }
...
function fileExportHasPortals({
  ...
  const ast = parseileAst(absPath, parser);
  if (!ast) {
  ...
  // ind the component declaration for exportName
  let compPaths = [];
  ...
  traverse(ast, {
    ExportDefaultDeclaration(p) {
      ...
      if (
        t.isunctionDeclaration(decl) ||
        t.isArrowunctionExpression(decl) ||
        t.isunctionExpression(decl)
      ) {
        compPaths.push(p.get("declaration"));
      } else if (t.isIdentifier(decl)) {
        const bind = p.scope.getinding(decl.name);
    ...
    ExportNamedDeclaration(p) {
      ...
      if (p.node.declaration) {
        ...
        if (t.isunctionDeclaration(d) && d.id?.name === exportName) {
          compPaths.push(p.get("declaration"));
        }
        ...
        p.node.specifiers.forEach((s) => {
          if (
            ...
            const bind = p.scope.getinding(s.local.name);
            if (bind) compPaths.push(bind.path);
          }
        });
      }
    ...
    nodePath.traverse({
      JSXOpeningElement(op) {
        ...
        if (/^[A-Z]/.test(name || "")) {
          ...
          const binding = op.scope.getinding(name);
...
function usageIsCompositePortal({
  ...
  // Same-file binding?
  const binding = jsxPath.scope.getinding(elementName);
  if (binding && binding.path) {
    ...
    binding.path.traverse({
      JSXOpeningElement(op) {
        ...
        if (/^[A-Z]/.test(name || "")) {
          const innerinding = op.scope.getinding(name);
          if (innerinding && innerinding.path) {
            innerinding.path.traverse(this.visitors);
          }
        }
      },
    });
  ...
  if (binding && binding.path && binding.path.isImportSpecifier()) {
    ...
    const filerom =
      ...
      state.file?.sourceileName ||
      __filename;
    const absPath = resolveImportPath(from, filerom);
...
const babelMetadataPlugin = ({ types: t }) => {
  ...
  const insertMetaAttributes = (openingEl, attrsToAdd) => {
    ...
    if (spreadIndex === -) {
      ...
      openingEl.attributes.splice(spreadIndex, , ...attrsToAdd);
    }
  };
  ...
  function pathIsDynamicComponent(path, visited = new WeakSet()) {
    ...
    if (
      path.isunctionDeclaration() ||
      path.isunctionExpression() ||
      path.isArrowunctionExpression()
    ) {
    ...
    if (path.isIdentifier()) {
      const binding = path.scope.getinding(path.node.name);
      if (binding) {
    ...
    if (path.isJSXElement() || path.isJSXragment()) {
      return pathHasDynamicJSX(path);
    }
  ...
  function fileExportIsDynamic({ absPath, exportName }) {
    ...
    function evaluatePath(nodePath) {
      ...
      if (
        nodePath.isunctionDeclaration() ||
        nodePath.isunctionExpression() ||
        nodePath.isArrowunctionExpression()
      ) {
      ...
      if (nodePath.isIdentifier()) {
        const binding = nodePath.scope.getinding(nodePath.node.name);
        if (binding) {
      ...
      if (nodePath.isJSXElement() || nodePath.isJSXragment()) {
        if (pathHasDynamicJSX(nodePath)) {
          dynamic = true;
        }
        return;
    ...
    traverse(ast, {
      ...
      ExportNamedDeclaration(p) {
        ...
        if (p.node.declaration) {
          const decl = p.node.declaration;
          if (t.isunctionDeclaration(decl) && decl.id?.name === exportName) {
            evaluatePath(p.get("declaration"));
            return;
          }
        ...
        p.node.specifiers.forEach((s) => {
          ...
          if (t.isIdentifier(s.local)) {
            const binding = p.scope.getinding(s.local.name);
            if (binding) {
              evaluatePath(binding.path);
            }
          }
  ...
  function componentindingIsDynamic({ binding, state }) {
    if (!binding || !binding.path) return false;
    const bindingPath = binding.path;

    if (INDING_DYNAMIC_CACHE.has(bindingPath.node)) {
      return INDING_DYNAMIC_CACHE.get(bindingPath.node);
    }
    ...
    if (bindingPath.isImportSpecifier()) {
      ...
      INDING_DYNAMIC_CACHE.set(bindingPath.node, result);
      return result;
    }
    ...
    if (bindingPath.isImportNamespaceSpecifier()) {
      INDING_DYNAMIC_CACHE.set(bindingPath.node, false);
      return false;
    }
  ...
  return {
    ...
    visitor: {
      ...
      // or stamp attributes when wrapping would break Radix/loating-UI.
      JSXElement(jsxPath, state) {
        ...
        const excludedComponents = new Set([
          ...
          "Link",
          "NavLink",
          // Portal-based primitives/triggers (Radix/loating-UI)
          "Sheet",
          "SheetContent",
          "SheetOverlay",
          "SheetPortal",
        ...
        if (!isDynamic) {
          const binding = jsxPath.scope.getinding(elementName);
          if (binding) {
            isDynamic = componentindingIsDynamic({ binding, state });
          }
        }
        ...
        // [OK] Normal case: wrap with display: contents and preserve an existing key
      ...
      JSXOpeningElement(jsxPath, state) {
        ...
        const lineNumber = jsxPath.node.loc?.start.line || ;
